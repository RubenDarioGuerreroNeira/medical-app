...
export class OSMService {
  ...
  async buscarFarmaciasEnTachira(): Promise<Location[] | null> {
    try {
      ...
      const data = await this.fetchWithRetry(`${url}?${params.toString()}`);

      // Validar la respuesta directamente aquí, como lo hacía el método `validateResponse`
      if (!data) {
        throw new Error(
  ...
  private async fetchWithRetry(url: string): Promise<any> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        // axios lanza un error para códigos de estado no 2xx, por lo que si llegamos aquí, está bien.
